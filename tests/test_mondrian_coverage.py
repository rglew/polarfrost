...
from polarfrost.mondrian import mondrian_k_anonymity, mondrian_k_anonymity_polars
...
def test_mondrian_with_single_column():
    ...
    assert len(result) >= 1
    assert all(count >= 2 for count in result['count'].to_list())
    assert result['age'].to_list() == ['25-25', '35-35'] or result['age'].to_list() == ['35-35', '25-25']
